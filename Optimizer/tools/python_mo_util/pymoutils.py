...
def compute_incr_hv(approximation_set, reference_point):
    """
        ...
        """
    incr_hv_val = np.array([0.0] * approximation_set.shape[0])
    for i in range(len(incr_hv_val)):
        incr_hv_val[i] = compute_pyhv(
            approximation_set[: i + 1], reference_point)
    return incr_hv_val
...
def compute_gd(approximation_set, reference_set):
    ...
    return libgd.gd(
        approximation_set,
        reference_set,
        approximation_set.shape[0],
        reference_set.shape[0],
        approximation_set.shape[1],
    )
...
def compute_incr_gd(approximation_set, reference_set):
    ...
    libgd.incr_gd(
        incr_gd,
        np.ascontiguousarray(approximation_set),
        np.ascontiguousarray(reference_set),
        approximation_set.shape[0],
        reference_set.shape[0],
        approximation_set.shape[1],
    )
...
def paretofront_cao(in_array):
    ...
    libpf.pf_cao(
        bool_array,
        np.ascontiguousarray(in_array.T),
        in_array.shape[0],
        in_array.shape[1],
    )
    return bool_array
...
def paretofront(in_array):
    ...
    libpf.pf_selective(
        bool_array, np.ascontiguousarray(
            in_array), in_array.shape[0], in_array.shape[1]
    )
    return bool_array
